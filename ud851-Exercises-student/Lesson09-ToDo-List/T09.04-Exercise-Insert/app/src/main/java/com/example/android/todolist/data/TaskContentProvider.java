...
public class TaskContentProvider extends ContentProvider {
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        final SQLiteDatabase writableDatabase = mTaskDbHelper.getWritableDatabase();
        Uri newUri;
        ...
        switch (sUriMatcher.match(uri)) {
            case TASKS:
                Long id = writableDatabase.insert(TaskEntry.TABLE_NAME, null, values);
                if (id > 0) {
                    newUri = ContentUris.appendId(uri.buildUpon(), id).build();
                } else {
                    throw new SQLException("Falied to insert the row int " + uri);
                }
                break;
            default:
                Log.e("Error unknown URI", "unknown URI");
