...
public class AddTaskActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEditTextTask=(EditText) findViewById(R.id.editTextTaskDescription);
        mPriority = 1;
    }
    ...
    public void onClickAddTask(View view) {
        ...
        String value = mEditTextTask.getText().toString();
        if (TextUtils.isEmpty(value)) {//value.trim().isEmpty()
           return;
        }
        // TODO (7) Insert new task data via a ContentResolver
        ContentValues contentValues = new ContentValues();
        contentValues.put(TaskEntry.COLUMN_DESCRIPTION,value);
        contentValues.put(TaskEntry.COLUMN_PRIORITY,mPriority);

        Uri backUri = getContentResolver().insert(TaskEntry.CONTENT_URI, contentValues);
        // TODO (8) Display the URI that's returned with a Toast
        // [Hint] Don't forget to call finish() to return to MainActivity after this insert is complete
        if (backUri != null) {
            Toast.makeText(this,backUri.toString(), Toast.LENGTH_SHORT).show();
        }
