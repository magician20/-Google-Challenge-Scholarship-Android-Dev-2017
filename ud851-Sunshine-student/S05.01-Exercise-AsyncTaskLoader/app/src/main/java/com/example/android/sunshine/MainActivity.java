...
public class MainActivity extends AppCompatActivity implements ForecastAdapterOnClickHandler, LoaderManager.LoaderCallbacks<String[]> {
    ...
    public static final int WEATHER_ID_LOADER = 1;
    ...
    private void loadWeatherData() {
        ...
        if (!TextUtils.isEmpty(location)) {
            args.putString(JSON_LOCATION, location);
        }else{
            //mean no data to load into bundle loader
            showErrorMessage();
            return;
        }
        LoaderManager  loaderManager= getLoaderManager();
        Loader<String[]> loader = loaderManager.getLoader(WEATHER_ID_LOADER);
        if (loader== null) {
            loaderManager.initLoader(WEATHER_ID_LOADER, args, this);
        } else {
            loaderManager.restartLoader(WEATHER_ID_LOADER, args, this);
        }
    }
    ...
    public Loader<String[]> onCreateLoader(int id, final Bundle args) {
        return new AsyncTaskLoader<String[]>(this) {
            ...
            public String[] loadInBackground() {
               String location = args.getString(JSON_LOCATION);
