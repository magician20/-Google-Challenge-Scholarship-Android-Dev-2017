...
//       (6) Move the cursor to the appropriate position
        ...
        //Set values
        int id = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry._ID));
        forecastAdapterViewHolder.itemView.setTag(id);

        long date = mCursor.getLong(mCursor.getColumnIndexOrThrow(WeatherContract.WeatherEntry.COLUMN_DATE));
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext, date, true);
        double miniTemp = mCursor.getFloat(mCursor.getColumnIndexOrThrow(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        double maxTemp = mCursor.getFloat(mCursor.getColumnIndexOrThrow(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
        String highAndLowTemperature =   SunshineWeatherUtils.formatHighLows(mContext, maxTemp, miniTemp);

        int weatherID = mCursor.getInt(mCursor.getColumnIndexOrThrow(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
        ...
        String weatherSummary = dateString + " - " + weatherDescription + " - " + highAndLowTemperature;
